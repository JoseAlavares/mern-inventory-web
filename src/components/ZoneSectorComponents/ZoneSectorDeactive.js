...
const ZoneModalDeactivate = ({ idZone, id, openDeactivate, handleModalDeactivate, refreshTable }) => {    
    ...
    const deactivate = async () => {
        try {
            const result = await axios({
                method: 'PUT',
                url: `${API_ENDPOINT}/api/v1/zone/${id}`,
                headers: HEADERS
            })
            console.log(result)
            if(!result) {
    ...
    return (
        <MDBContainer>
          <MDBModal 
            ...
            <MDBModalHeader 
                toggle={handleModalDeactivate}>
                Cambiar el estado de la bodega                    
            </MDBModalHeader>
            <MDBModalBody>
                <MDBContainer>                    
                    <h3>¿Esta seguro de cambiar estado de la bodega?</h3>
                </MDBContainer>
            </MDBModalBody>
            <MDBModalFooter>
                <MDBBtn 
...
export default ZoneModalDeactivate