...
import { getPayloadToken, cleanCharacters, onlyNumbers } from '../../utils/functions'
import "react-datepicker/dist/react-datepicker.css"
import "./stock-styles.css"

const StockAdd = ({warehouseId, childProductId, productBatchId, openAdd, handleModalAdd, refreshTable}) => {	
	...
	const getZones = async () => {
		try {
			const result = await axios({
				method: 'GET',
				url: `${API_BACKEND}/api/v1/warehouse-zones/${warehouseId}`,
				headers: HEADERS
			})
	...
	const addStock = async () => {
		...
		const quantity = onlyNumbers(input.quantity) ? parseFloat(input.quantity) : 0

		if(!input.quantity) errors.push('El campo cantidad no puede estar vacio')
		if(quantity <= 0) errors.push('L cantidad debe ser mayor a cero')
		if(quantity > 10000) errors.push('La cantidad debe ser menor a 10,000.00')
		...
		input.id_child_product = childProductId	
	...
	return (
		<MDBContainer>
			<MDBModal isOpen={openAdd} toggle={handleModalAdd} centered>
				...
				<MDBModalFooter>
					<MDBBtn className="cancel-button" onClick={handleModalAdd}>
						Cancelar
					</MDBBtn>
